...
<?php
	...
	$query="select * from recipe where rtitle LIKE '%".$keyword."%' OR rSteps LIKE '%".$keyword."%' or rid in (select rid from recipeTag natural join Tag where  tname LIKE '%".$keyword."%')";

// 	$query="SELECT distinct rid, uname, rtitle, rServNum, rCookTime, rSteps, rtime  FROM Recipe natural join RecipeTag natural join Tag WHERE rtitle LIKE '%".$keyword."%' OR rSteps LIKE '%".$keyword."%'  OR tname LIKE '%".$keyword."%' ORDER BY rid" ;
// 	echo $query;
...
<nav class="navbar navbar-default" role="navigation">
    <div class="container-fluid">
        <div class="navbar-header">
            <a class="navbar-brand" href="index.php"> Welcome to Cookbook!</a>
        </div>
	...
	if($num == 0) echo '<div class="alert alert-danger"><strong> "No recipe is found. Please try again in <a href="index.php"> home page.</a>"</strong>'.'</div> ';
		...
		$rtitle = $value["rtitle"];
			...
			if (mysqli_num_rows($resultim)==1){
				...
				$iId = "<img src= 'http://localhost/proj2/Images/".$MyPhoto."'width = 300/>";
			} else {
				$iId = '';
			}
			?>
